...
case $1 in
    ...
    if [ "$IS_BACK" != "" ] || [ "$IS_NOT_ALL" = "" ]; then
      ...
      if [ -f pyproject.toml ]; then
        process="
          ...
          source $(poetry env info --path)/bin/activate >/dev/null 2>&1"
  ...
  "${PROCESS[1]}" )
    ...
    if [ "$IS_BACK" != "" ] || [ "$IS_NOT_ALL" = "" ]; then
      ...
      process="pybabel extract -F babel.cfg -o $LOCALES_DIR/messages.pot . >/dev/null 2>&1"
      run_process "Updating translation template of backend" "$process"
      process="pybabel update -i $LOCALES_DIR/messages.pot -d $LOCALES_DIR >/dev/null 2>&1"
      run_process "Updating translation files of backend" "$process"

      process="alembic revision --autogenerate -m 'autogenerated' >/dev/null 2>&1"
