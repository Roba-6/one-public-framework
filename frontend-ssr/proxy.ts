...
function getAdminSegment(): string {
  ...
  if (!/^[A-Za-z0-9_-]+$/.test(value)) {
    throw new Error("UI_ADMIN_PATH may only contain letters, numbers, underscores, and hyphens.");
  }
  return value;
}
...
export function proxy(request: NextRequest) {
  ...
  const isInternalAdminRewrite = request.headers.get("x-one-public-ui-admin-rewrite") === "1";
